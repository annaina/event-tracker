...
//
// Given an RGB frame and a batch of events from the same moment, we find
// the (sx, sy, ox, oy) transform that best aligns event edges with RGB
// edges. The transform is:
//
//   u_evt = sx * u_rgb + ox
//   v_evt = sy * v_rgb + oy
//
// We do this by:
//   1. Running Canny on the RGB frame to get edges
//   2. Accumulating events into a binary image (events = edges)
//   3. Grid-searching over (sx, sy, ox, oy) in 3 passes to maximize
//      the fraction of event-edge pixels that land on RGB edges
//
// This replaces the need for hardcoded intrinsics — just point two cameras
// at the same scene and it figures out the mapping.
...
// Build a binary edge image from events (events fire on brightness changes,
// so accumulating them gives something very similar to a Canny edge map).
...
// Score an alignment candidate. Returns fraction of event-edge pixels that
// land on (dilated) RGB edges when warped with the given transform.
...
//
// If wide=true, the search range is much larger (for unknown cameras).
// If wide=false, it searches tightly around the initial guess.
    ...
    bool wide = false)
{
    ...
    // -- Build downsampled images for each pass --
    ...
    // Fine pass: half-res, use min(4, n_frames) frames
    ...
    // Sub-pixel pass: full-res, average over all frames for best accuracy
    ...
    // --- Pass 2: fine (half-res, multiple frames) ---
    ...
    // --- Pass 3: sub-pixel (full-res, all frames) ---
